...
export function AdminAppointments() {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-2xl font-bold text-foreground mb-2">Appointment Statistics & Overview</h2>
                <p className="text-muted-foreground">Monitor all appointments across the system</p>
            </div>
            ...
            {stats && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <Card className="border border-border bg-accent/15">
                        <CardContent className="p-5">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-sm font-medium text-accent-foreground">Pending</p>
                                    <p className="text-3xl font-bold text-foreground">{stats.byStatus.pending}</p>
                                </div>
                        ...
                        <CardContent className="p-5">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-3xl font-bold text-primary">{stats.byStatus.completed}</p>
                                </div>
                        ...
                        <CardContent className="p-5">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-3xl font-bold text-destructive">{stats.byStatus.cancelled}</p>
                                </div>
                        ...
                        <CardContent>
                            <div className="grid grid-cols-2 gap-3">
                                {Object.entries(stats.byStatus).map(([key, count]) => {
                                    ...
                                    const sc = STATUS_CONFIG[scKey] || { bg: 'bg-muted', color: 'text-foreground', label: key };
                                    return (
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {appointments.map(apt => {
                                        const sc = STATUS_CONFIG[apt.status] || { bg: 'bg-muted', color: 'text-foreground', label: apt.status };
                                        return (
                                            <tr key={apt._id} className="border-b last:border-0 hover:bg-muted">
                                                <td className="py-3">
                                                    <div>{new Date(apt.finalDate || apt.preferredDate).toLocaleDateString()}</div>
                                                    <div className="text-xs text-muted-foreground">{apt.finalTime || apt.preferredTime}</div>
                                                </td>
                                                <td className="py-3">
                                                    <Badge className="bg-muted text-foreground border-0">{TYPE_LABELS[apt.appointmentType] || apt.appointmentType}</Badge>
                                                </td>
